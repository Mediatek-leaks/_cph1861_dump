   ...
   if [ ! `getprop vold.decrypt`  ] ; then
     if [ ! "$(df /data | grep tmpfs)" ] ; then
        if [ -e "/dev/block/bootdevice/by-name/userdata" ] ; then
           ...
           if [ -e "/dev/block/platform/bootdevice/by-name/userdata" ]  && [ -e "/sys/class/BOOT/BOOT/boot/boot_mode" ] && [ $(cat /sys/class/BOOT/BOOT/boot/boot_mode) == "1" ] ; then
...
function copy_mtklog() {
    ...
    mv /data/system/users/0/faceunlock/* sdcard/mtklog/faceunlock/
